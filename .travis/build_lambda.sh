...
zip -rq $TRAVIS_BUILD_DIR/dist/databridge-etl-tools.zip . -x psycopg2/**\* psycopg2_binary-2.8.2.dist-info/**\

echo "Zip together previous zip file and psycopg2 compiled files"
cd $TRAVIS_BUILD_DIR/
zip -urq $TRAVIS_BUILD_DIR/dist/databridge-etl-tools.zip psycopg2-3.6

echo "Zip together previous zip file and lambda function"
cd $TRAVIS_BUILD_DIR/lambda
zip -gq $TRAVIS_BUILD_DIR/dist/databridge-etl-tools.zip index.py